...
def run_amplifiers(phases, values):
    output = 0
    for i in range(5):
        output = run_machine(values, input_values=[phases[i], output])[0]
    return output
...
if __name__ == '__main__':
    ...
    for phases in itertools.permutations([0,1,2,3,4], 5):
        values = VALUES[:]
        output = run_amplifiers(phases, values)
        logging.debug(f'{phases}, {output}')
        if best < output:
            best = output
            best_phases = phases
    print(f'Part a: {best}')
